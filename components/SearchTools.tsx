...
import { ArrowLeftRight, ArrowUpRight, CalendarDays, FileText, Globe2, Quote, Search } from 'lucide-react';
...
import { buildGoogleSearchUrl, getDateRangeError, parseSearchSyntax, upsertSearchOperator } from '../lib/search-syntax';
...
function DateRange({draft,onDraft}:Pick<Props,'draft'|'onDraft'>) {
  const feedbackId = 'date-range-feedback';
  ...
  const error = getDateRangeError(draft);
  return <div className="tool-body date-tool"><CalendarDays className="hero-tool-icon" strokeWidth={1.2}/><div className="tool-content"><h2>Find it in a moment in time.</h2><div className="date-fields"><label>After<input type="date" aria-label="Search after date" value={/^\d{4}-\d{2}-\d{2}$/.test(after)?after:''} max={before||undefined} aria-invalid={Boolean(error)} aria-describedby={error?feedbackId:undefined} onChange={event=>onDraft(upsertSearchOperator(draft,'after',event.target.value))}/></label><span>—</span><label>Before<input type="date" aria-label="Search before date" value={/^\d{4}-\d{2}-\d{2}$/.test(before)?before:''} min={after||undefined} aria-invalid={Boolean(error)} aria-describedby={error?feedbackId:undefined} onChange={event=>onDraft(upsertSearchOperator(draft,'before',event.target.value))}/></label></div>{error&&<p id={feedbackId} className="field-error" role="status">{error}</p>}</div></div>;
}
...
export default function SearchTools({mode,draft,onDraft}:Props){switch(mode){case'flights':case'hotels':case'shopping':return <TravelTools mode={mode} draft={draft}/>;case'dino':return <DinoGame/>;case'convert':return <Converter draft={draft}/>;case'weather':case'define':case'movies':case'compare':return <KnowledgeTools mode={mode} draft={draft} onDraft={value=>onDraft(value,mode)}/>;case'calculate':case'timer':case'stopwatch':case'metronome':case'color':return <UtilityTools mode={mode} draft={draft}/>;case'science':case'game':case'play':return <PlayTools mode={mode} draft={draft}/>;case'finance':case'news':return <MarketTools mode={mode} draft={draft}/>;case'places':case'video':case'images':return <DiscoveryTools mode={mode} draft={draft}/>;case'documents':return <Documents draft={draft} onDraft={onDraft}/>;case'site':return <Website draft={draft} onDraft={onDraft}/>;case'date':return <DateRange draft={draft} onDraft={onDraft}/>;case'precise':return <Precision draft={draft} onDraft={onDraft}/>;default:return <div className="general-tool"><Search size={23}/><p>The whole web is a good place to start.</p><OutLink query={draft}>Search Google</OutLink></div>;}}
