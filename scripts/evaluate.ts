...
async function loadCases(split: Split): Promise<EvalCase[]> {
  ...
  for (const current of splits) {
    ...
    if (data.schemaVersion !== 1 || data.dataset !== "search-v3" || data.split !== current || !Array.isArray(data.cases)) throw new Error(`Invalid ${current} dataset.`);
